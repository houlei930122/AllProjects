...
<script>
  //映射 mapState()/mapMutation()/mapActions()/mapGetters()
  //通过这些映射方法可以让大家少敲几个字，避免对$store的直接访问
  import {mapStates, mapMutation,mapActions,mapGetters} from 'vuex'

  export default {
    computed: {
      ...mapStates('user',['isLogin']),  //使用this.isLogin 就可以直接访问到user.js里面的state了
